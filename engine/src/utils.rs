...
macro_rules! get_gl_error {
    () => {
        ...
        match a {
            ...
            1282 => println!("GL ERROR({}, {}): Invalid operation, set when the state of a command is not legal for its given params", file!(), line!()),
            ...
            1286 => println!("GL ERROR({}, {}): Invalid framebuffer operation, set when reading or writing to a framebuffer that is not complete", file!(), line!()),
            _ => {}
        }
    };
}